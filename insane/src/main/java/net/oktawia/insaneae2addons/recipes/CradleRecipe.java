...
public class CradleRecipe implements Recipe<CradleContext> {
    ...
    @Override
    public boolean matches(CradleContext ctx, Level level) {
        return pattern.matches(level, ctx.origin(), ctx.facing());
    }

    @Override
    public ItemStack assemble(CradleContext ctx, RegistryAccess regs) {
    ...
    public RecipeSerializer<?> getSerializer() {
        return InsaneRecipes.CRADLE_SERIALIZER.get();
    }

    @Override
    public RecipeType<?> getType() {
        return InsaneRecipes.CRADLE_TYPE.get();
    }
